...
function buildChoicesHTML() {
  const labels = ['A', 'B', 'C'];
  const shuffled = shuffleArray(answerPool);
  // Store answer index as data attribute to avoid JSON-in-HTML escaping bugs
  return shuffled.map((ans, i) => `
    <button class="choice-card"
      data-quality="${ans.quality}"
      data-label="${ans.label.replace(/"/g,'&quot;')}"
      data-message="${ans.message.replace(/"/g,'&quot;').replace(/'/g,'&#39;')}"
...
function makeChoice(btn) {
  ...
  // Show watch next on best answer; always show reset button
  if (quality === 'best') {
